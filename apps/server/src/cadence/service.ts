...
/**
 * Replace the room's proposal list and broadcast it.
 *
...
export async function propose(
	...
	let saved = await room.auth.storage.cadence.replaceAll(room.room, items, new Date());
...
export async function push(room: Room, ws: Socket, msg: Request<Wire.Push>): Promise<void> {
	...
	let resolved = await resolveServer(room, ws.data.principalId, item.mcpServer);
	if ("reason" in resolved) {
		...
		return reply(ws, msg.rid, { kind: "session:error", ts: 0, message: resolved.reason });
	}
